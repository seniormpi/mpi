...
def make_labels():
    ...
    for feat_num in range(1, 10):
        for model_num in range(1, 112):
            for orn in range(1, 7):
                lst.append([str(feat_num) + "_" + str(model_num) + "_" + str(orn), feat_num])
    for feat_num in range(10, 17):
        for model_num in range(1, 143):
            for orn in range(1, 7):
                lst.append([str(feat_num) + "_" + str(model_num) + "_" + str(orn), feat_num])
    for feat_num in range(17, 22):
...
data_dir = '.\\data\\feature_data_equally'
...
model.save_weights('model_features_mod.h5')
...
# Confusion Matrix
model.load_weights('model_features_mod.h5')
...
data_dir = '.\\data\\cmb_data_equally'
df = make_labels()
partition = data_process(df)
...
model1.save_weights("model_mpi_mod.h5")
...
data_dir = '.\data\fin_data_equally'
df = make_labels()
partition = data_process(df)
...
data_dir = '.\\data\\cmb_data_machinability_equally'
df = make_labels()
partition = data_process(df)
...
model1.save_weights('model_classify_mach_non_mach_mod.h5')
